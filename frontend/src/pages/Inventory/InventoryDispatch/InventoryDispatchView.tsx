...
function dispatchTypeBadge(t?: string) {
	...
	if (s === "QUOTATION") {
		return (
			<Badge bg='warning' text='dark' style={{ borderRadius: 999, padding: "6px 10px" }}>
				QUOTATION
			</Badge>
		);
	}
...
export default function InventoryDispatchView() {
	const { id } = useParams();
			...
			>
				...
				<Card.Body style={{ background: "#f7fbfa", padding: 18 }}>
					...
					>
						<div className='d-flex align-items-center gap-2'>
							...
							<div style={{ fontSize: 13, color: "#6c757d" }}>
								<b style={{ color: "#212529" }}>Inventory Dispatch View:</b>{" "}
								read-only mirror of warehouse dispatch data
							</div>
						</div>
					...
					<Row className='g-3 mb-3'>
						...
						<Col md={6} lg={4}>
							<Field label='Quotation No' value={selected.quotationNo || "-"} />
						</Col>
