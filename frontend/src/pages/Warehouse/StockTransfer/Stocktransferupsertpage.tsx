...
const TransferSchema = Yup.object({
	...
	transferFromWarehouse: Yup.string().required(
		"Transfer from warehouse required",
	),
	transferToWarehouse: Yup.string().required("Transfer to warehouse required"),
...
export default function StockTransferUpsertPage() {
	...
	// masters — same selector pattern as WarehouseInwardUpsertPage
			...
			if (getStockTransferThunk.fulfilled.match(res)) {
				...
				setInitialValues({
					transferDate: t.transferDate
						...
						: "",
					transferFromWarehouse: t.transferFromWarehouse || "",
					transferToWarehouse: t.transferToWarehouse || "",
					remarks: t.remarks || "",
					items:
	...
	// Derive a lookup function that works against the live Formik value of
	// transferFromWarehouse (passed in at render time from values)
	const buildWarehouseInventory = (fromWarehouse: string) => {
		if (!fromWarehouse || !inventories) return [];
		const key = fromWarehouse.trim().toLowerCase();
	...
	const getAvailableQty = (fromWarehouse: string, itemName: string) => {
		const inv = buildWarehouseInventory(fromWarehouse);
		return (
			inv.find(
				(i: any) =>
					String(i.itemName || "")
	...
	return (
		...
		>
			...
			<div className='d-flex justify-content-between align-items-center mb-3'>
				<div>
					<h5 className='m-0'>
						{isEdit ? "Edit Stock Transfer" : "Create Stock Transfer"}
					</h5>
					<div style={{ fontSize: 13, color: "#6c757d", marginTop: 2 }}>
						{isEdit
							? "Update transfer details (only DISPATCHED transfers can be edited)"
				...
				<Button
					variant='light'
					size='sm'
					onClick={() => nav("/warehouses/stock-transfer")}
					style={{
			...
			{loading ? (
				...
				<Formik
					...
					onSubmit={async (values, { setSubmitting }) => {
						...
						// client-side available qty guard
						for (let i = 0; i < values.items.length; i++) {
							const it = values.items[i];
							const avail = getAvailableQty(
								values.transferFromWarehouse,
								it.itemsName,
							);
						...
						const payload = {
							...
							transferFromWarehouse: values.transferFromWarehouse,
							transferToWarehouse: values.transferToWarehouse,
						...
						if (isEdit) {
							const res = await dispatch(
								updateStockTransferThunk({ id: id!, payload }),
							);
							if (updateStockTransferThunk.fulfilled.match(res)) {
								toast.success("Stock transfer updated");
								nav("/warehouses/stock-transfer", { replace: true });
							} else {
							...
							const res = await dispatch(createStockTransferThunk(payload));
							if (createStockTransferThunk.fulfilled.match(res)) {
								toast.success("Stock transfer created — inventory reserved");
								nav("/warehouses/stock-transfer", { replace: true });
							} else {
				...
				>
					{({
						...
						// ── Filtering helpers (identical logic to WarehouseInwardUpsertPage) ──
						...
						// Items filtered by subcategory AND restricted to what exists in
						// the FROM warehouse inventory
						const getFilteredItems = (subCategoryName: string) => {
							if (!subCategoryName) return [];
							const whInv = buildWarehouseInventory(
								values.transferFromWarehouse,
							);
							...
							return (masterItems || []).filter((it: any) => {
								...
								const nm = String(it.itemName || it.name || it.label || "")
									...
									.toLowerCase();
								const inWarehouse =
									!values.transferFromWarehouse || whItemNames.has(nm);
							...
							<Form onSubmit={handleSubmit}>
								...
								>
									<div className='d-flex align-items-center gap-2 mb-2'>
										...
										<div style={{ fontWeight: 700 }}>Transfer Details</div>
									</div>
									...
									<Row className='g-3'>
										...
										<Col md={4}>
											...
											<Form.Select
												...
												value={values.transferFromWarehouse}
												onChange={(e) => {
													setFieldValue(
														"transferFromWarehouse",
														e.target.value,
													);
													// reset items when FROM warehouse changes
													setFieldValue("items", [emptyItem()]);
												}}
												isInvalid={
													!!touched.transferFromWarehouse &&
													!!errors.transferFromWarehouse
												}
												style={{ borderRadius: 8 }}
											>
											...
											<Form.Control.Feedback type='invalid'>
												{errors.transferFromWarehouse as any}
											</Form.Control.Feedback>
										</Col>
										...
										<Col md={4}>
											...
											<Form.Select
												...
												value={values.transferToWarehouse}
												onChange={handleChange}
												isInvalid={
													!!touched.transferToWarehouse &&
													!!errors.transferToWarehouse
												}
												style={{ borderRadius: 8 }}
											>
												<option value='' disabled>
												...
												{warehouseOptions
													.filter(
														(w: any) => w.name !== values.transferFromWarehouse,
													)
													.map((w: any) => (
														<option key={w.id} value={w.name}>
															{w.name}
											...
											<Form.Control.Feedback type='invalid'>
												{errors.transferToWarehouse as any}
											</Form.Control.Feedback>
										</Col>
								...
								>
									<div className='d-flex align-items-center gap-2 mb-3'>
										...
										{values.transferFromWarehouse && (
											...
											>
												...
												<strong>{values.transferFromWarehouse}</strong>)
											</span>
										)}
									</div>
									...
									{!values.transferFromWarehouse && (
										<Alert variant='info' style={{ fontSize: 13 }}>
											Please select a <strong>Transfer From Warehouse</strong>{" "}
												...
												<Row className='g-3'>
													...
													<Col md={6}>
														...
														<Form.Select
															...
															disabled={!values.transferFromWarehouse}
															isInvalid={
